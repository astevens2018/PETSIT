...
console.log(mapDash)
if (mapDash) { // don't try to build a map if there's no div#map to inject in
  const map = new GMaps({ el: '#map-dashboard', lat: 0, lng: 0 });
  const marker = JSON.parse(mapDash.dataset.marker);
  console.log(marker)
  map.addMarkers(marker);
